...
Here the signature ``N, axes=(0, 1, 2), dtype=np.float`` tells us that the
created ``fft`` instance is *planned* such as to transform 
any 3D array of shape ``N`` and type ``np.float``. Furthermore, we plan to 
transform axis 2 first, and then 1 and 0, which is exactly the reverse order of 
``axes=(0, 1, 2)``. Mathematically, it corresponds to

.. math::
    :label: 3d_rfftn

    \boldsymbol{\hat{u}} = \mathcal{F}_0(\mathcal{F}_1( \mathcal{F}_{2}(\boldsymbol{u})),

and if the entire array could only fit in the memory of one single processor,
then that would be all there was to it. However, this array is quite large and 
we need to distribute it to several processors. And for distributed arrays
we need to use both global redistributions and serial transforms. 
With one group of processors, the simplest possible complete transform would 
then be
...
This corresponds to a slab distribution, where axis 0 is distributed on the
...
that realigns the global data such that it is aligned in axes 0 and 2. 
...
determines that axes 0 and 2 should use one processor each, i.e., they should
be non-distributed.
...
:math:`u_{j_0/P,j_1,j_2}` can transform both axes 1 and 2 simultaneously,
...
Note that collapse can also be configured through the ``axes`` keyword, 
using::
