...
class HDF5Writer(object):
    """Class for writing data in mpi4py-fft to HDF5 format
    ...
    def write_tstep(self, tstep, u, spectral=False):
        ...
        self._write_group(self.name, u, tstep, spectral)
        ...
        for i, j in enumerate(sl):
            if isinstance(j, slice):
                ...
                if j >= s[i].start and j < s[i].stop:
                    ...
                    inside *= 0
        self._write_slice_group(self.name, slname, ndims, sp, u, sl, sf, inside, tstep)
